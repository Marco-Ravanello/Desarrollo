...
export function TaskList({ initialTasks, userId }: { initialTasks: any[], userId: string }) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await addTaskAction(userId, newTask);
    setNewTask("");
  };
            ...
            <Card key={t.id} className={`transition-all ${t.status === 'COMPLETADA' ? "opacity-50" : ""}`}>
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button onClick={() => toggleTaskAction(t.id, t.status)}>
                    {t.status === 'COMPLETADA' ? (
                  ...
                  <span className={`text-sm ${t.status === 'COMPLETADA' ? "line-through text-slate-500" : ""}`}>
                    {t.title}
                  </span>
                </div>
                <Button variant="ghost" size="icon" onClick={() => deleteTaskAction(t.id)} className="text-slate-400 hover:text-red-500">

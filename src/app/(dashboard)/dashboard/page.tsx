...
import { getDashboardStats, parseDateRange } from "@/services/dashboard";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Users, FileText, ArrowRightLeft, ShoppingBag, Activity, User, Calendar, Car, CheckCircle2, ShieldAlert } from "lucide-react";
...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const { from, to } = parseDateRange(params.range, params.from, params.to);
  const stats = await getDashboardStats({ from, to });

  const mainCards = [
    { title: "Familias Registradas", description: "Total en base de datos", value: stats.peopleCount, icon: Users, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-950/30", trend: "En el período" },
    { title: "Casos Activos", description: "Abiertos o en proceso", value: stats.activeCases, icon: FileText, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/30", trend: "En el período" },
    { title: "Tareas Pendientes", description: "Asignadas para hoy", value: stats.todayTasks, icon: CheckCircle2, color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950/30", trend: "Vencimiento en período" },
        ...
        <Card className="bg-white/75 dark:bg-card/75 backdrop-blur-md border border-border/40 shadow-municipal">
          <CardHeader className="flex flex-row items-center gap-2">
            ...
            <div className="space-y-1">
              ...
              <CardDescription>Últimas acciones realizadas en la plataforma para este período.</CardDescription>
            </div>
          </CardHeader>
          <CardContent>
            {stats.recentActivity.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                ...
                <p>No hay actividad registrada aún para este período.</p>
              </div>
            )}
          </CardContent>
        </Card>
